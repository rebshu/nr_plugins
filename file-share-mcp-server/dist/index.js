...
function getConfig() {
    ...
    const pagesBaseUrl = process.env.PAGES_BASE_URL || `https://${owner}.github.io/${repo}`;
    if (!token) {
 ...
 */
function buildPagesUrl(filePath) {
...
const UploadInputSchema = z
    ...
    file_path: z
        .string()
        .min(1)
        .describe("Absolute local path to the file to upload (e.g. /path/to/report.html)"),
    destination: z
        .string()
        .optional()
        .describe("Optional destination path within the repo (e.g. 'shared/reports/q1.html'). If omitted, uses the source filename."),
    folder: z
        .string()
        .optional()
        .describe("Optional subfolder to organize uploads (e.g. 'reports', 'overviews'). Prepended to the filename."),
    commit_message: z
        .string()
        .optional()
        .describe("Custom commit message. Defaults to 'Upload <filename> via file-share-mcp'."),
})
    .strict();
server.registerTool("file_share_upload", {
...
const ListInputSchema = z
    ...
    folder: z
        .string()
        .optional()
        .describe("Optional folder to list (e.g. 'reports'). Lists root if omitted."),
})
    .strict();
server.registerTool("file_share_list", {
    ...
    try {
        ...
        if (files.length === 0) {
            ...
            for (const file of files) {
                lines.push(`- **${file.name}** (${(file.size / 1024).toFixed(1)} KB) → ${file.url}`);
            }
        }
        return {
...
const DeleteInputSchema = z
    ...
    repo_path: z
        .string()
        .min(1)
        .describe("Path of the file within the repo to delete (e.g. 'reports/q1-report.html')"),
    commit_message: z
        .string()
        .optional()
    ...
    .strict();
server.registerTool("file_share_delete", {
...
async function main() {
    ...
    console.error(`file-share-mcp-server running (repo: ${config.owner}/${config.repo})`);
}
main().catch((error) => {
